...
use crate::models::{ExchangeBalance, ExchangeType, UserExchange};
use anyhow::Result;
use base64::Engine;
use reqwest::Client;
...
impl ExchangeService {
    ...
    async fn get_binance_balance(
        ...
        let signature = self.generate_binance_signature(&query_string, &user_exchange.secret_key);
    ...
    async fn get_okx_balance(&self, user_exchange: &UserExchange) -> Result<Vec<ExchangeBalance>> {
        ...
        let balances: Vec<ExchangeBalance> = balance_response
            .data
            .into_iter()
            .flat_map(|data| {
    ...
    async fn get_weex_balance(&self, user_exchange: &UserExchange) -> Result<Vec<ExchangeBalance>> {
        ...
        log::info!(
            "  Passphrase: {}",
            &user_exchange.passphrase.as_deref().unwrap_or("")
        );
