...
impl Scheduler {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        // 3. 翻仓用户即时祝贺任务
        tokio::spawn(async move {
            if let Err(e) = congratulation_scheduler.start_congratulation_monitor().await {
                error!("翻仓祝贺监控任务失败: {}", e);
            }
        });
    ...
    async fn execute_daily_top5_broadcast(&self) -> Result<()> {
        ...
        info!("✅ Top5排名消息生成成功 - 前{}名用户, 总参赛人数: {}", ranking_message.top_rankings.len(), ranking_message.total_participants);
        ...
        for entry in daily_rankings {
            if entry.is_doubled {
                ...
                if !congratulated_users.contains(&user_key) {
                    info!("🎉 发现新翻仓用户: {} ({})", entry.user_name, entry.exchange_type);
    ...
    // 发送Top5排名通知
    async fn send_top5_ranking_notification(&self, ranking_message: &crate::models::ranking::DingTalkRankingMessage) -> Result<()> {
    ...
    // 发送翻仓祝贺消息
    async fn send_congratulation_message(&self, entry: &crate::models::ranking::RankingEntry) -> Result<()> {
        let mut message_content = String::new();
