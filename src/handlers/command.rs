...
use crate::services::{DatabaseService, DingTalkBot, ExchangeService};
use anyhow::Result;
...
impl CommandHandler {
    ...
    pub async fn handle(&self, cli: Cli) -> Result<()> {
        match cli.command {
            Commands::AddUser { dingtalk_id, name } => self.add_user(&dingtalk_id, &name).await,
            Commands::AddExchange {
                dingtalk_id,
                exchange,
                api_key,
                secret_key,
            ...
            Commands::QueryBalance { dingtalk_id } => self.query_balance(&dingtalk_id).await,
            Commands::TriggerQuery => self.trigger_query().await,
            Commands::StartScheduler => self.start_scheduler().await,
        }
    }
    ...
    async fn add_exchange(
        ...
        // 验证API配置
        if let Err(e) = self
            .validate_exchange_config(&exchange_type, api_key, secret_key, passphrase)
            .await
        {
            println!("API配置验证失败: {}", e);
            return Ok(());
        ...
        let user_exchange = self
            .database
            .add_user_exchange(user.id, exchange_type, api_key, secret_key, passphrase)
            .await?;

        println!(
            "交易所配置添加成功: {} ({})",
            user_exchange.exchange_type, user_exchange.id
        );
    ...
    async fn query_balance(&self, dingtalk_id: &str) -> Result<()> {
        ...
        for user_exchange in user_exchanges {
            ...
            match self
                .exchange_service
                .get_account_balance(&user_exchange)
                .await
            {
                Ok(balances) => {
    ...
    async fn validate_exchange_config(
        &self,
